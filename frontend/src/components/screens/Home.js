...
const Home = () =>{
    ...
    if(data.length === 0 || state ==null  ) 
    {
        // console.log("if",data.length)
        return <h1>Create a Post2</h1>
    }
    if(data[0].postedBy ==null){
        console.log(loading)
        return <h1>Test</h1>
    }
    else if(loading==true && data[0].postedBy != null){
        // console.log("else",data.length,"loading",loading)
        data.map(item => console.log(item._id))
    return (
